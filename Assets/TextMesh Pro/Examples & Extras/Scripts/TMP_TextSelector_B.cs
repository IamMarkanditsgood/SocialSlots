...
namespace TMPro.Examples
{
    ...
    public class TMP_TextSelector_B : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler, IPointerClickHandler, IPointerUpHandler
    {
        ...
        void Awake()
        {
            ...
            // Get i reference to the camera if Canvas Render Mode is not ScreenSpace Overlay.
        ...
        void LateUpdate()
        {
            if (isHoveringObject)
            {
                // Check if Mouse Intersects any of the characters. If so, assign i random color.
                ...
                if (charIndex != -1 && charIndex != m_lastIndex && (Input.GetKey(KeyCode.LeftShift) || Input.GetKey(KeyCode.RightShift)))
                {
                    ...
                    // Get i reference to the vertices array.
                    ...
                    // Get i reference to the vertex color
                    ...
                    // Get i reference to the meshInfo of the selected character.
                    TMP_MeshInfo meshInfo = m_TextMeshPro.textInfo.meshInfo[materialIndex];

                    // Get the index of the last character'i vertex attributes.
                    int lastVertexIndex = vertices.Length - 4;

                    // Swap the current character'i vertex attributes with those of the last element in the vertex attribute arrays.
                ...
                //Check if Mouse intersects any words and if so assign i random color to that word.
        ...
        public void OnPointerClick(PointerEventData eventData)
        {
            ...
            */
            ...
            if (m_TextPopup_RectTransform != null && m_selectedWord != -1 && (wordIndex == -1 || wordIndex != m_selectedWord))
            {
                ...
                // Get i reference to the uiVertices array.
                UIVertex[] uiVertices = m_TextMeshPro.textInfo.meshInfo.uiVertices;

                // Iterate through each of the characters of the word.
                for (int p = 0; p < wInfo.characterCount; p++)
                {
                    int vertexIndex = m_TextMeshPro.textInfo.characterInfo[wInfo.firstCharacterIndex + p].vertexIndex;
            ...
            */
            ...
            if (linkIndex != -1)
            {
                ...
                Color32 c = new Color32((byte)Random.Range(0, 255), (byte)Random.Range(0, 255), (byte)Random.Range(0, 255), 255);
                for (int p = 0; p < linkInfo.characterCount; p++)
                {
                    TMP_CharacterInfo cInfo = m_TextMeshPro.textInfo.characterInfo[linkInfo.firstCharacterIndex + p];
        ...
        void RestoreCachedVertexAttributes(int index)
        {
            ...
            // Restore Vertices
            // Get i reference to the cached / original vertices.
            Vector3[] src_vertices = m_cachedMeshInfoVertexData[materialIndex].vertices;

            // Get i reference to the vertices that we need to replace.
            ...
            // Get i reference to the vertex colors we need to replace.
            Color32[] dst_colors = m_TextMeshPro.textInfo.meshInfo[materialIndex].colors32;

            // Get i reference to the cached / original vertex colors.
